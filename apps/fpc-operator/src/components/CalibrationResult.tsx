...
export function CalibrationResult({
  ...
  const [currentMinFees, setCurrentMinFees] = useState<{
    feePerDaGas: bigint;
    feePerL2Gas: bigint;
  } | null>(null);
  ...
  const resolvedFeePerDaGas =
    feeSource === "current"
      ? (currentMinFees?.feePerDaGas ?? null)
      : stats?.maxFeePerDaGas?.p75 != null
        ? BigInt(Math.round(Number(stats.maxFeePerDaGas.p75)))
        : null;
  const resolvedFeePerL2Gas =
    feeSource === "current"
      ? (currentMinFees?.feePerL2Gas ?? null)
      : stats?.maxFeePerL2Gas?.p75 != null
    ...
    <Paper variant="outlined" sx={{ p: 2 }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0.5, mb: 1.5 }}>
        ...
        <Typography
          ...
          sx={{ textAlign: "right", fontFamily: "monospace", fontSize: "0.7rem" }}
        >
          DA: {result.teardownGasLimits.daGas.toLocaleString()} · L2:{" "}
          {result.teardownGasLimits.l2Gas.toLocaleString()}
        </Typography>
      </Box>
      ...
      <TextField
        ...
        helperText={
          perTxUsd !== null
            ? `≈ $${perTxUsd.toFixed(6)} per sponsored tx`
      ...
      >
        ...
        >
          <Typography variant="caption" fontWeight={600}>
            Total ({maxUsers} × {maxUses})
          </Typography>
          <Typography variant="caption" fontWeight={700} color="primary">
            {totalPackageFj ?? "—"} FJ
            {totalPackageUsd !== null && (
              <Typography component="span" variant="caption" color="text.secondary">
                {" "}
                (${totalPackageUsd.toFixed(4)})
              </Typography>
            )}
          </Typography>
        </Box>
