...
export function BackupRestore({ mode = "full" }: BackupRestoreProps) {
  ...
  const networkMismatch =
    importData?.network && importData.network !== activeNetwork.id;
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Export your admin keys and FPC configuration to a JSON file. Store
        this file securely — it contains private keys.
      </Typography>
      <Alert severity="warning" sx={{ mb: 2 }}>
        The backup file contains your admin secret key and FPC secret key.
        Anyone with this file can control your admin account and FPC contract.
        Keep it safe.
      </Alert>
      <Button
        variant="contained"
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Replace the current configuration with a previously exported backup
        file. This will overwrite all current data and reload the page.
      </Typography>
      {importError && (
  ...
  return (
    <Dialog open={open} onClose={applying ? undefined : onCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        {networkMismatch && (
          <Alert severity="warning" sx={{ mb: 2 }}>
            This backup was exported from network "{data.network}".
