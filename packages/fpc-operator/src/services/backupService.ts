...
export async function parseAndValidateBackup(
  ...
  | { data: BackupData; errors?: undefined }
...
export async function applyBackup(
  ...
  if (
    data.calibrationIndices &&
    Object.keys(data.calibrationIndices).length > 0
  ) {
    localStorage.setItem(
      CALIBRATION_CACHE_KEY,
      JSON.stringify(data.calibrationIndices),
    );
  }
