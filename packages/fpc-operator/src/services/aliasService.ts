...
const contractsKey = (networkId: string) =>
  `${CONTRACTS_KEY_PREFIX}:${networkId}`;
const sendersKey = (networkId: string) =>
  `${SENDERS_KEY_PREFIX}:${networkId}`;
...
export const getStoredContracts = (networkId: string) =>
  read(contractsKey(networkId));
...
export function addStoredContract(networkId: string, entry: StoredAliased) {
  const list = getStoredContracts(networkId).filter(
    (c) => c.address !== entry.address,
  );
  list.push(entry);
...
export const getStoredSenders = (networkId: string) =>
  read(sendersKey(networkId));
...
export function addStoredSender(networkId: string, entry: StoredAliased) {
  const list = getStoredSenders(networkId).filter(
    (s) => s.address !== entry.address,
  );
  list.push(entry);
