...
import { connectWallet, getConnectedAccount, switchChain, getWalletChainId, requestAccountSwitch, revokeWalletPermissions } from '../services';
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    getConnectedAccount().then(addr => { if (addr) setAccount(addr); });
    getWalletChainId().then(id => { if (id != null) setChainId(id); });
  }, []);
  ...
  return (
    <WalletContext.Provider value={{ account, chainId, isConnecting, wrongChain, error, connect, switchAccount, disconnect }}>
      {children}
