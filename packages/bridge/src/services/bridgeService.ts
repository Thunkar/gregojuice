...
export async function bridgeDouble(params: {
  ...
  if (currentAllowance < totalAmount) {
    const approveHash = await walletClient.writeContract({
      ...
      abi: erc20WriteAbi,
      functionName: "approve",
      args: [addresses.feeJuicePortal, totalAmount],
      account,
      chain,
    });
    await publicClient.waitForTransactionReceipt({ hash: approveHash });
  ...
  const ephSecretHashHex =
    `0x${ephSecret.secretHash.toString(16).padStart(64, "0")}` as Hex;
  const ephRecipientHex = (
    ephemeralRecipient.startsWith("0x")
      ? ephemeralRecipient
      : `0x${ephemeralRecipient}`
  ) as Hex;
  ...
  const ephBridgeHash = await walletClient.writeContract({
    ...
    abi: portalWriteAbi,
    functionName: "depositToAztecPublic",
    args: [ephRecipientHex, ephemeralAmount, ephSecretHashHex],
    account,
    chain,
  });
  ...
  const mainSecretHashHex =
    `0x${mainSecret.secretHash.toString(16).padStart(64, "0")}` as Hex;
  const mainRecipientHex = (
    mainRecipient.startsWith("0x") ? mainRecipient : `0x${mainRecipient}`
  ) as Hex;
  ...
  const mainBridgeHash = await walletClient.writeContract({
    ...
    abi: portalWriteAbi,
    functionName: "depositToAztecPublic",
    args: [mainRecipientHex, mainAmount, mainSecretHashHex],
    account,
    chain,
  });
